...
<div class="w-full bg-white p-6 rounded shadow">
    <h2 class="text-2xl font-bold mb-2">Kategori Surat &gt;&gt; Tambah</h2>
    <p class="text-gray-700 mb-6">
        Tambahkan data kategori baru. Jika sudah selesai, jangan lupa untuk mengklik tombol "Simpan".
    </p>
    ...
    <form action="{{ route('kategori.store') }}" method="POST">
        ...
        <div class="mb-4">
            <label for="id" class="block text-sm font-medium text-gray-700">ID (Auto Increment)</label>
            <input type="text" id="id" name="id" 
                value="{{ ($lastId ?? 0) + 1 }}" readonly
                class="mt-1 block w-full bg-gray-100 border border-gray-300 rounded px-3 py-2 text-gray-600 cursor-not-allowed" />
        </div>

        <!-- Nama Kategori -->
        <div class="mb-4">
            <label for="nama" class="block text-sm font-medium text-gray-700">Nama Kategori <span class="text-red-500">*</span></label>
            <input type="text" id="nama" name="nama" value="{{ old('nama') }}"
                class="mt-1 block w-full border border-gray-300 rounded px-3 py-2 focus:ring-blue-500 focus:border-blue-500" 
                placeholder="Contoh: Undangan, Pemberitahuan">
            @error('nama')
        ...
        <div class="mb-6">
            ...
            <textarea id="keterangan" name="keterangan" rows="4"
                class="mt-1 block w-full border border-gray-300 rounded px-3 py-2 focus:ring-blue-500 focus:border-blue-500"
                placeholder="Tuliskan keterangan kategori (opsional)">{{ old('keterangan') }}</textarea>
            @error('keterangan')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
